...
parser.add_argument('-w', '--load_weights', type=str, default='weights\\hybridnets-d3_262_8750.pdparams')
...
with paddle.no_grad():
    ...
    if seg_mode == BINARY_MODE:
        ...
        seg_mask = paddle.argmax(seg, 1)  # 三张区域类别分数张量在每个像素上比较(axis=1)，取最大值，0：背景 1：可行驶区域 2：车道线
        seg_mask_list.append(seg_mask)
    else:
