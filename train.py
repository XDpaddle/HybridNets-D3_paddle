...
def get_args():
    ...
    # 'checkpoints\\bdd100k\\hybridnets-d3_336_8750.pdparams'
    ...
    parser.add_argument('--is_train', type=boolean_string, default=False,
                        help='Automatic Mixed Precision training')
...
def train(opt):
    ...
    if opt.freeze_det: 
        for i, param in enumerate(model.model.regressor.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
        for i, param in enumerate(model.model.classifier.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
        for i, param in enumerate(model.model.anchors.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
    ...
    if opt.freeze_seg:
        for i, param in enumerate(model.model.bifpndecoder.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
        for i, param in enumerate(model.model.segmentation_head.named_parameters()):  # 返回所有最底层的参数nn.Conv2D，nn.BatchNorm2D，nn.Linear
            # print(param)
            param[1].trainable = False
            # print(param)
                ...
                for data in training_generator:
                    ...
                    try:
                        ...
                        if step%1==0:
                            ...
                            # print(step, paddle.device.cuda.memory_allocated())
